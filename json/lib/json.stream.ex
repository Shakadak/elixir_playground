defmodule Json.Stream do
  ...
  def encode!(x) when is_list(x),
    do:
      Stream.concat([
        ["["],
        Stream.concat(Stream.intersperse(Stream.map(x, &encode!/1), [","])),
        ["]"]
      ])
  ...
  def encode!(x) when is_function(x, 2),
    do:
      Stream.concat([
        ["["],
        Stream.concat(Stream.intersperse(Stream.map(x, &encode!/1), [","])),
  ...
  def encode!(%{__struct__: y} = x),
    do: raise("Structs are not handled: #{inspect(x)} (#{inspect(y)})")
